...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (token && user) { redirectByRole(user.role); return; }
...
async function handleRegister() {
  ...
  try {
    const res  = await fetch('/api/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, password, studentId }) });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Registration failed');
    localStorage.setItem('att_token', data.token);
    localStorage.setItem('att_user', JSON.stringify(data.user));
    const s = document.getElementById('register-success');
    s.textContent = '✅ Account created! Redirecting…'; s.style.display = 'block';
